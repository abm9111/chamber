...
- **Debt payment is still vector-only.** `search` and `ask` retrieve hybrid (vector ∪ FTS5); `proposeDebtPayment` does not, and passes `hybrid: false` explicitly. Its auto-pay gate fires on `best.score >= 0.35`, a constant calibrated on the cosine scale, and hybrid ranking would change which row lands at index 0 without changing what `score` means — so the threshold would quietly start meaning something else. The consequence is that a debt whose supporting passage is only findable by a rare literal term still cannot be paid automatically; a human running `chamber search` can find it and attach it.
- **`retrievedBy: "both"` can mean "matched one stopword".** The label reports membership of the bm25 candidate list, not that the match was meaningful. In a small corpus every term looks rare, so a passage sharing only `the` is a lexical candidate with a `lexicalScore` of ~0 and a `both` label. It changes no ranking (its lexical contribution is zero) but it reads as more agreement between the legs than there is.
- **Lexical retrieval is consistent but near-useless for CJK text.** `vector_document_fts` uses FTS5's default `unicode61` tokenizer, which splits on whitespace and punctuation and treats an unbroken run of Chinese, Japanese or Korean characters as a *single* token. A CJK sentence is therefore indexed as one giant term and queried as the same one giant term, so the leg is at least self-consistent — an exact copy of a sentence matches it — but nothing shorter does: searching a two-character word inside a CJK passage matches nothing, and every CJK passage looks like a hapax to `queryRarity`. Latin, Cyrillic and Greek text is unaffected; the vector leg carries CJK retrieval on its own. Fixing it needs a segmenting tokenizer (`icu`, or a trigram index), which is a schema change and a re-index.
...
- ~~**Retrieval is vector-only, so a rare literal token is unfindable.**~~ **Fixed by hybrid retrieval.** Chunking fixed *which* text got embedded; it could not fix what MiniLM has no representation for. `gbrain Hindsight memory bolt-ons revealed preference` returned memory-systems surveys at ≈0.50 and never returned the note containing the words, while `vector_document_fts` — populated, in step, 20,447 rows — answered `MATCH 'gbrain'` correctly and nothing queried it. `search` and `ask` now retrieve the union of both legs, ranked by `0.7*cosine + 0.3*lexicalStrength`; see `.superpowers/sdd/hybrid-retrieval-report.md` for the fusion argument and the before/after measurements.
