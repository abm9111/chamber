...
- Empty states: no report → setup instructions with the exact cron line;
  report clean → "No drift. N conclusions checked <time>."
...
**Fixtures are generated by real `chamber verify --json` runs** against the
demo corpus (regeneration script checked in) — the parser is tested against
...
The plugin repo's fixture-regeneration script uses `npx -y @bu7umaid/chamber`
(dev-time only — the plugin at runtime has zero Chamber dependency).
